...
with st.sidebar:
    ...
    st.divider()
    st.subheader("Feed Ration (Add your ingredients)")
    ...
    for i in range(int(num_feeds)):
        ...
        cost = st.number_input(f"Cost per kg (currency)", key=f"cost_{i}", min_value=0.0, value=0.20, step=0.01)
...
#
# Main area
if st.sidebar.button("🔄 Calculate My Margins", type="primary"):
...
#
    ...
    if new_results['feed_cost_per_liter'] > new_milk_price * 0.5:
        st.info(
            "💡 **Tip:** Feed costs exceed 50% of your milk income. Optimizing your ration formulation could boost profitability.")



#

# calculator.py (or inside app.py)
def calculate_margin(cow_count, avg_milk_ltrs, milk_price, feed_items, other_costs):
    """
    feed_items: list of dicts, e.g., [{"name":"Silage", "qty":10, "cost":2.5}, ...]
    """
